...
def graph1():
    ...
    plt.ylim(150, 210)
    plt.bar(name, height, color='pink', hatch='/', ec='red')
    plt.xticks(name, rotation=45, size=8, color='g')
    for idx, h in enumerate(height):
        plt.text(idx, h+1, h, ha='center', color='b', size=8)
...
@app.route('/graph2') #학생별평균점수
def graph2():
    df['평균'] = df.apply(lambda row:row['국어':'사회'].mean(), axis=1) #학생별평균점수
    name = df['이름']
    avg = df['평균']
    plt.figure(figsize=(10, 3))
    plt.ylim(0, 100)
    plt.bar(name, avg, color='pink', hatch='/', ec='yellow')
    plt.xticks(name, rotation=45, size=8, color='g')
...
def graph3():
    ...
    plt.ylim(160, 200)
    plt.bar(labels, values, color='pink', hatch='/', ec='yellow', width=0.5)
