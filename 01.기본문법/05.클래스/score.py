...
class Score(Student):
    ...
    def info_print(self): #성적 객체의 정보를 출력하는 메서드(클레스 안에있는 함수)
        super().info_print() #super 대신 스튜던트인포프린트 해도됨
    ...
    def dict(self): #딕셔너리로 변환하는 메서드(함수)
        return {'no':self.no, 'name':self.name, 'kor':self.kor, 'eng':self.eng, 'mat':self.mat,
                'avg': (self.kor+self.eng+self.mat)/3,
                'result':self.result()}
